...
public class Authenticator {
     ...
     */
    public MinecraftProtocol authenticate(ProxyInfo proxy) {
     ...
     */
    public static ProxyInfo getAuthProxy() {
        if (Hydrazine.settings.hasSetting("authproxy")) {
            ...
            if (Hydrazine.settings.getSetting("authproxy").contains(":")) {
                try {
                    ...
                    proxy = new ProxyInfo(ProxyInfo.Type.HTTP, new InetSocketAddress(parts[0], Integer.parseInt(parts[1])));
                } catch (Exception e) {
                ...
                if (authFile.exists()) {
                    ...
                    proxy = Objects.requireNonNull(authFactory.getProxies(ProxyInfo.Type.HTTP))[r.nextInt(Objects.requireNonNull(authFactory.getProxies(ProxyInfo.Type.HTTP)).length)];
                } else {
                    System.out.println(Hydrazine.errorPrefix + "Invalid value for switch '-ap'");
